...
| **セットアップ** | `npx 3am-cli init --mode automatic --provider anthropic` | `npx 3am-cli init --mode manual --provider claude-code` |
| **ブリッジ** | 不要 | 必要。別ターミナルで `npx 3am-cli bridge` を立ち上げる |

**API キーがあるなら → `automatic` モードが本番向けの経路です:**
...
> **よくあるミス:** `--mode manual --provider anthropic` は推奨されません。manual モードは、サーバー側の API キーを持っていないときに使うモードです。`ANTHROPIC_API_KEY` があるなら、`--mode automatic` の方がシンプルです。
...
ブリッジはデプロイ済みの Receiver に WebSocket（Cloudflare Workers、Node.js）またはロングポール（Vercel）で接続し、診断リクエストをローカルで処理します。認証トークンは `npx 3am-cli deploy` が保存した認証情報から自動で読み取られます。
...
**対応 LLM プロバイダ** — `npx 3am-cli init` 実行時に選択します（デフォルト: `anthropic`）。未選択のまま進めた場合は、下記の優先順位で最初に使えるプロバイダを自動検出します:
...
**モデルの推奨:** 診断品質とコストのバランスでは Claude **Sonnet** が一番おすすめ。節約したいなら Haiku でも問題ありません。Opus 4.6 / 4.7 はこのワークロードには過剰です。
...
最近アクティビティがあったインシデントは保持されます。保持期間を過ぎたオープン中のインシデントは自動でクローズされ、クリーンアップされます。
...
そこで Vercel にデプロイする場合、`3am init` は `package.json` のビルドスクリプト中の `"next build"` を `"next build --webpack"` に書き換え、本番ビルドでは Webpack を強制的に使うようにします。dev サーバ（`next dev`）には影響しません。
...
```bash
npx 3am-cli init                                    # アプリに OTel 計装を組み込む
npx 3am-cli init --mode automatic --provider anthropic   # automatic モード（API キー利用）
...
npx 3am-cli bridge --receiver-url <url>             # WebSocket またはロングポールでリモート Receiver につなぐ
```

`init` のフラグ: `--api-key`、`--mode automatic|manual`（エイリアス: `auto`）、`--provider anthropic|openai|claude-code|codex|ollama`、`--model`、`--bridge-url`、`--lang en|ja`、`--no-interactive`

`bridge` のフラグ: `--port`（デフォルト 4269）、`--receiver-url`（リモート WebSocket/ロングポールの接続先。省略時は認証情報から自動検出）
