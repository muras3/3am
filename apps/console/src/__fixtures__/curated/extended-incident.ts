...
export const extendedIncidentPending: ExtendedIncident = {
  ...
  headline: "",
  chips: [
    { type: "critical", label: "Checkout failures confirmed" },
    { type: "external", label: "Stripe in scope" },
  ],
  action: {
    text: "Open Evidence Studio and inspect the first failing checkout trace.",
    rationale: "The impact is confirmed, but the causal explanation is still converging. The fastest safe next step is to verify the first failing request path and dependency behavior.",
    doNot: "Do not treat the first visible dependency as root cause until traces and logs agree on the same trigger.",
  },
  rootCauseHypothesis: "",
  causalChain: [],
  operatorChecks: [
    "Open the first failing trace for POST /checkout",
    "Confirm whether Stripe spans appear in the failing path",
    "Check logs for repeated dependency-specific error signatures",
  ],
  impactSummary: {
    startedAt: "2026-03-20T14:23:15Z",
    fullCascadeAt: "",
  ...
  blastRadius: [
    { target: "payment-service", status: "critical", impactValue: 0.42, label: "42%" },
    { target: "checkout-api", status: "degraded", impactValue: 0.18, label: "18%" },
  ],
  confidenceSummary: {
    label: "Early signal",
    value: 0.22,
    basis: "Impact is confirmed, cause is not yet cross-validated",
    risk: "The first correlated dependency may be incidental rather than causal",
  },
  evidenceSummary: { traces: 9, traceErrors: 4, metrics: 2, logs: 37, logErrors: 11 },
  state: {
    diagnosis: "pending",
    baseline: "ready",
