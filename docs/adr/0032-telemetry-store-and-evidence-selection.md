...
OTel ingest → SpanBuffer (L1: in-memory, max 1000, TTL 5min)
            → TelemetryStore (L2: Postgres, 48h retention)
```
...
### 3. Postgres + Drizzle ORM、type 別テーブル

TelemetryStore は StorageDriver (ADR 0013) とは別の subsystem として実装する。
...
| `telemetry_metrics` | service, environment, name, startTimeMs, summary (JSONB), ingestedAt | (service, name, startTimeMs) UNIQUE, (ingestedAt) |
| `telemetry_logs` | service, environment, timestamp, severity, body, bodyHash, attributes (JSONB), traceId, spanId, ingestedAt | (service, timestamp, bodyHash) UNIQUE, (ingestedAt), (traceId) |

Drizzle ORM を使用 (ADR 0024 と同じ技術選定)。ただし StorageDriver schema の peer であり、StorageDriver interface には含めない。

**CF D1 との関係**: TelemetryStore は Postgres 前提。CF D1 (SQLite) のサポートは non-goal。StorageDriver (ADR 0013) の cross-platform 方針は incident canonical store に限定し、TelemetryStore は Vercel Postgres / セルフホスト Postgres をターゲットにする。
...
Incident 作成時に、packetizer が選別した結果を **別テーブル** (`incident_evidence_snapshots`) に保存する。

```
incident_evidence_snapshots:
  incidentId (FK → incidents)
  snapshotType: "traces" | "metrics" | "logs" | "platform_events"
  data (JSONB)
  createdAt
```

- Snapshot は packet rebuild の入力（ADR 0030 の rawState の役割を引き継ぐ）
- TelemetryStore の 48h TTL 後もスナップショットは incident が存在する限り保持
...
|----------------|---------------|
| `IncidentRawState` | TelemetryStore (生データ) + `incident_evidence_snapshots` (選別済み) |
| rawState への append | TelemetryStore への ingest (incident 非依存) |
| rawState からの packet rebuild | curated snapshot からの packet rebuild |
...
StorageDriver interface は変更しない。TelemetryStore は StorageDriver とは別の subsystem。
- StorageDriver: incident canonical store (cross-platform: D1 / Vercel Postgres)
- TelemetryStore: OTel raw data store (Postgres only)
...
TelemetryStore は Postgres only。CF Workers + D1 環境では TelemetryStore を利用できない。これは意図的な制約:
- D1 (SQLite) は z-score baseline 計算、48h TTL、Evidence Studio の複雑なクエリに適さない
- StorageDriver (incident canonical store) は引き続き D1 をサポート
- 将来的に CF Workers 環境向けの TelemetryStore adapter (例: Hyperdrive 経由の Postgres) を検討可能
