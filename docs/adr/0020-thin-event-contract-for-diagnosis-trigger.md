...
> ADR 0030 により packet は incident raw state からの derived view となり、新しい signals / evidence が attach されるたびに rebuild される。`packet_id` は **latest canonical view** を指す stable identifier であり、creation-time snapshot を指すものではない。
>
