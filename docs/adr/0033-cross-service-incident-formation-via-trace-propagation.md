...
2. **マッチしない場合、traceId による cross-service lookup を実行**:
   ...
   b. 既存 open incident の **incident-bound trace set** と intersection を取る
   c. intersection が空でなければ、その incident に merge する
3. **`MAX_CROSS_SERVICE_MERGE` ガードは維持** — ADR 0017 の上限 (=3) を超える merge は行わない

#### Incident-bound trace set の取得方法

**`pointers.traceRefs` は使用しない。** `traceRefs` は ADR 0032 で MAX_TRACE_REFS=30 に cap された retrieval 用 pointer であり、incident membership の canonical set ではない。31 件目以降の trace に乗った cross-service anomaly を見落とすため、formation key としては lossy。

代わりに、incident-bound な全 traceId を canonical source から取得する:

| 実装段階 | Canonical source | 方法 |
|---------|-----------------|------|
| **現行** (rawState 存在時) | `rawState.spans` | `new Set(rawState.spans.map(s => s.traceId))` — full span object から抽出 |
| **Step 4-5 後** (rawState 廃止後) | `spanMembership: string[]` | `new Set(spanMembership.map(id => id.split(':')[0]))` — compact ref から traceId 部分を抽出。DB query 不要 |

`spanMembership` は traceId:spanId の compact ref set（ADR 0032 Step 4-5 プラン DJ-2）であり、incident に紐づく全 span の ID を保持する。cap なし。
...
**現行実装** (rawState 経由): snapshot-builder の `computeWindowAndScope()` が `rawState.spans` の全 serviceName を収集。merge により web + notification-svc の span が同一 rawState に入れば、両方の metrics/logs が自動的に選別対象になる。

**Step 4-5 後** (rawState 廃止後): `telemetryScope.memberServices` が canonical source に変わる。`expandTelemetryScope()` により merge 先 incident の memberServices に新サービスが追加され、snapshot-builder は memberServices + dependencyServices で TelemetryStore を query する。rawState への依存は解消される。
...
`sharedTraceIds` の計算には、新 batch の anomalous span の traceId と既存 incident の **incident-bound trace set** の intersection を使う（上述の canonical source 参照）。`pointers.traceRefs` は cap されているため使用しない。

### パフォーマンス考慮

- **現行** (rawState): `rawState.spans` の traceId 抽出は in-memory。open incident 数は通常 1-5 件。`getRawState()` は formation 判定時に既に呼ばれている（attach 判定のため）ので追加 I/O なし
- **Step 4-5 後** (spanMembership): compact ref の split は O(n)。1000 spans × 80 chars ≈ 80KB の JSON parse は十分高速。open incident 数が少ないため問題にならない
