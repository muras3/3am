...
| 0012 | Phase 1 では `@3am/otel` を作らない。Phase 2 で必須 |
...
packages/
  core/               ← @3am/core（npm 公開）
  diagnosis/          ← @3am/diagnosis（npm 公開）
  cli/                ← @3am/cli（npm 公開）
...
#### `packages/diagnosis`（npm 公開: `@3am/diagnosis`）
...
#### `packages/cli`（npm 公開: `@3am/cli`）

`packages/diagnosis` の薄いラッパー。CLI インターフェースの提供のみを担う（ADR 0015）。

- `diagnose <packet.json>` — `@3am/diagnosis` を呼び出して結果を出力
- `replay <packet.json>` — 過去 packet の再診断（開発・検証用）

GitHub Actions は `npx @3am/cli diagnose` をシェルアウトで呼ぶ。
...
| パッケージ名 | 対象 | 公開フェーズ |
|-------------|------|------------|
| `@3am/core` | schema・型定義 | Phase 1 |
| `@3am/diagnosis` | LLM 診断エンジン | Phase 1 |
| `@3am/cli` | CLI（diagnosis の薄いラッパー） | Phase 1 |
| `@3am/otel` | OTel wrapper | Phase 2（ADR 0012） |
...
- `@3am/core` を公開することで、サードパーティが独自の診断ランタイムや Console を実装できる拡張性が生まれる
