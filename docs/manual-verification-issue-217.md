...
1. Run `npx 3am-cli init --mode automatic --provider anthropic`.
2. Start Receiver with `npx 3am-cli local`.
3. Trigger a demo incident with `npx 3am-cli local demo`.
...
1. Run `npx 3am-cli init --mode manual --provider claude-code`.
2. Start Receiver with `npx 3am-cli local`.
3. Start the local bridge with `npx 3am-cli bridge`.
