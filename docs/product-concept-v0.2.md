...
- **注意**: FAST_MODE（warmup/steady_state を数秒に短縮）での実行のため信号量が少ない。フルスケール run での再検証前は参考値として扱う。
...
```
App (OTel SDK) → Receiver (異常検知) → webhook → 診断ランタイム (LLM診断) → Slack通知
```
...
Receiverからwebhookを受けてOTelデータを取得し、v5プロンプトでLLM診断を実行する。実行環境の選択肢は以下のとおりで、v0.2時点では確定していない：

| 環境 | メリット | デメリット |
|------|----------|------------|
| GitHub Actions | LLMキーをSecrets管理、無料枠あり | GitHub依存、Vercel/CFユーザー全員が使うわけではない |
| ローカルCLI | 最も摩擦が少ない | 常時起動が前提 |
...
異常検知は現時点で**アルゴリズム未確立**。以下の2つの方向性を仮説として持つ：

1. **ルールベース**: エラー率・レイテンシ閾値の固定ルール。実装が単純で偽陽性率の見積もりが立てやすい
2. **統計的閾値**: 時系列ベースラインからの逸脱検出。精度は高いが実装・チューニングコストが大きい

MVP では**ルールベースを第一仮説**として検証する。偽陽性率の許容基準を先に定義してからアルゴリズムを実装する。
...
- [ ] **偽陽性基準の定義**: 正常系シナリオ（障害なしの通常トラフィック）で偽陽性率の許容閾値を数値で定義し、その基準でテストを実施する
- [ ] **異常検知第一仮説の確定**: ルールベースか統計的閾値か、MVP で試す手法を1つ決める
- [ ] **フルスケールrunでの再検証**: FAST_MODE ではなく本来の所要時間（warmup込み13分/シナリオ）で 5 シナリオを実行し、9.0/10相当の精度を確認する
...
- [ ] フルスケールrunでの再検証（Phase 1 開始条件）
- [ ] 偽陽性テスト（Phase 1 開始条件）
- [ ] 異常検知第一仮説の確定（Phase 1 開始条件）
