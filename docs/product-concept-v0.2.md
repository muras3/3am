...
- 対象データは `OTel traces / OTel logs / OTel metrics` に限定（platform logs はスコープ外）
- 全文検索型の汎用ログ基盤は作らない
...
### Incident Formation と Packetization

Receiver は複数の signal / alert をそのまま個別通知しない。  
同じ incident に属すると判断できるものは、まず **1つの incident** に束ねてから packet を作る。

LLM を使わずに行うこと:

- time window の切り出し
- service / route / deployment / dependency の scope narrowing
- related alerts の incident formation
- changed metrics の抽出
...
Receiverからwebhookと `incident packet` を受け取り、v5プロンプトでLLM診断を実行する。

Phase 1 では **GitHub Actions を推奨構成**とする。  
...
| 環境 | メリット | デメリット |
|------|----------|------------|
| GitHub Actions | 推奨構成。Secrets管理が自然 | GitHub依存 |
| ローカルCLI | replay / 開発 / デバッグ / evaluation に向く | 本番導線の主経路にはしない |
| Vercel Functions / CF Workers | 将来の追加候補 | Phase 1 では採用しない |

packet 生成の本体は Receiver 側に置き、GitHub Actions と CLI は同じ packet schema を消費する。
...
### 診断プロンプト（v5 — 7ステップ）
`v5` は raw dump 全体を読むためのプロンプトではなく、incident formation と packetization を経た `incident packet` を読むための推論レイヤーとして使う。
...
| **MEDIUM** | incident formation が弱く、1障害が複数通知に分裂する | Datadog/Dynatrace のような高度な相関は未実装 | related alerts を LLM なしで incident 単位に束ねる前段ロジックを実装する |
| **MEDIUM** | GitHub 依存が一部ユーザーに合わない | GitHub Actions を推奨構成に固定した | CLI parity を維持しつつ、将来の追加 runtime を検討する |
...
### Phase 1: MVP
- [x] 技術スタック決定（TypeScript + Hono + React/Vite + Turborepo）
