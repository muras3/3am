...
> 目的: 3am の診断品質を改善するために、ローカルコンテナで再現可能な障害シナリオ実行環境を構築する。
...
PostgreSQL。最初のシナリオでは `web` が軽く接続するだけ（red herring 用の `db_connection_count` を自然に生成する）。2本目の `db_migration_lock_contention` で本格的に使う。

責務:

- `web` からの接続受け入れ
- connection count の自然な変動（`web` の connection pool metric として OTel 経由で収集）
- 将来: migration lock / long-running query の再現
...
継続的にトラフィックを流す HTTP サーバー。`scenario-runner` から制御 API 経由で負荷プロファイルを切り替える。
...
- flash sale 的な急増
- シナリオ時刻に合わせた負荷変化
- `POST /__admin/profile` で runtime 切り替え
- `GET /__admin/state` で現在状態を返却
...
`scenario-runner` の後段スクリプトとして実行する。別コンテナにはしない。

責務:

- collector 出力の回収
- `events.json` 整形
- `ground_truth.json` テンプレートからの生成
- 最終 fixture ディレクトリ生成

実装上は `scenario-runner` コンテナ内の別スクリプトで十分。シナリオ実行終了後に `scenario-runner` が呼び出す。
...
validation/
  ...
  apps/
    ...
    mock-stripe/
  tools/
    scenario-runner/   # artifact-writer もここに含む
  otel/
...
```json
{
  ...
  "primary_root_cause": "Aggressive retry logic retries Stripe 429 responses at fixed 100ms intervals without backoff, causing retry fan-out that exhausts the shared 16-worker pool",
  "contributing_root_causes": [
    "Flash sale traffic spike increased Stripe API call volume beyond rate limits",
    "Shared worker pool serves both payment and non-payment routes"
  ],
  "detail": {
    "component": "checkout-orchestrator/retry-logic",
    "trigger_signal": "Stripe API returning 429 status codes followed by retry fan-out spans",
    "failure_mode": "Retry storm saturates worker pool, cascading to site-wide 504"
  },
  "recommended_actions": [
    "Implement exponential backoff with jitter on Stripe retry logic",
    "Add circuit breaker on Stripe calls",
    "Separate worker pools for payment and non-payment routes"
  ],
  "t_first_symptom_oracle": "2026-03-06T10:05:00.000Z",
...
2. `postgres`（最小テーブル定義のみ）
3. `web`
4. `mock-stripe`
5. `otel-collector`
6. `loadgen`（HTTP サーバー + 制御 API）
7. `scenario-runner`（artifact-writer スクリプトを含む）
8. `summary.json` 生成
